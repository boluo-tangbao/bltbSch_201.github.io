...
test('production city archive renders existing content, automatic image descriptions and text addresses', async ({ page }) => {
  ...
  await expect(page.locator('video, .video-pending, .pros-cons, .demo-badge')).toHaveCount(0)
  if (place.cover) {
    await expect(page.locator('.detail-photo img')).toHaveAttribute('alt', `${place.city} · ${place.name}`)
    await expect.poll(() => page.locator('.detail-photo img').evaluate((img: HTMLImageElement) => img.naturalWidth)).toBeGreaterThan(0)
  }
