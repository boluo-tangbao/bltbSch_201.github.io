...
for (const item of ['src', 'public', 'index.html', 'guide']) cpSync(resolve(root, item), resolve(target, item), { recursive: true })
...
const places = Array.from({ length: 28 }, (_, index) => ({
  ...
  video: index === 0 ? { url: 'https://example.com/video', title: '虚构视频介绍', excerpt: '这是我的视频文字节选测试。\n<script>不是可执行内容</script>', startSeconds: 63, endSeconds: 95, clip: null } : null,
}))
