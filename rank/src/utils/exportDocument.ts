...
import { cityColor, formatTime, navigationUrl } from './cities'
...
export async function exportDocument(site: Site, places: Place[], allPlaces: Place[], updates: Update[], scope: string, updatedAt: string | null) {
  const imagePaths = [...new Set(places.flatMap(p => [...(p.cover ? [p.cover] : []), ...p.gallery.map(g => g.src)]))]
  ...
  const content = tiers.map(t => `<section><h2 style="background:${t.color}">${t.label}</h2>${places.filter(p => p.tier === t.id).map(p => {
    ...
    const video = p.video
    const segment = video?.startSeconds !== undefined ? `${formatTime(video.startSeconds)}${video.endSeconds !== undefined ? '–' + formatTime(video.endSeconds) : ' 起'}` : '片段时间未记录'
    return `<article style="border-color:${cityColor(p.city, site.cityColors)}"><p>${e(p.city)}${site.rankWithinTier ? ` · 档内第 ${rank} 名` : ''}${p.isDemo ? ' · 演示' : ''}</p><h3>${e(p.name)}</h3>${p.cover ? `<img src="${images.get(p.cover)}" alt="${e(p.coverAlt)}">` : '<p>暂无图片</p>'}<p class="prose">${e(p.summary)}</p><p>到访：${e(p.visitedAt || '未记录')} · 内容更新：${e(p.updatedAt)}</p>${p.tags.length ? `<p>标签：${p.tags.map(e).join(' / ')}</p>` : ''}${video ? `<h4>视频介绍：${e(video.title)}</h4><p>${segment}</p><blockquote class="prose">${e(video.excerpt || '文字节选待补充。')}</blockquote><p><a href="${e(video.url)}">打开原视频</a>${video.clip ? ` · <a href="${e(new URL(assetUrl(video.clip), location.origin).href)}">观看已发布片段</a>` : ''}</p>` : '<p>视频介绍待补充。</p>'}${paragraph('亮点', p.pros)}${paragraph('槽点', p.cons)}${paragraph('图文补充', p.details)}${p.gallery.map(g => `<figure><img src="${images.get(g.src)}" alt="${e(g.alt)}"><figcaption>${e(g.alt)}</figcaption></figure>`).join('')}${p.location ? `<h4>位置与导航</h4><p>${e(p.location.address || '详细地址待补充')}<br>WGS84：${p.location.lat}, ${p.location.lng}</p><a href="${e(navigationUrl(p, true)!)}">在高德地图查看</a>` : '<p>位置待补充。</p>'}<p><a href="${website}guide/#/place/${p.id}">查看最新介绍</a></p></article>`
