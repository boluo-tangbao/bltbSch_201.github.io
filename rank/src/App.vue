...
<template>
  ...
  <header class="site-header"><div class="header-inner">
    ...
    <nav aria-label="主导航"><a href="#/" :aria-current="isHome ? 'page' : undefined">体验榜单</a><a :href="homeUrl">个人主页 <span aria-hidden="true">↗</span></a></nav>
  </div></header>
  ...
  <main id="main-content" ref="main" tabindex="-1" class="page-shell">
    <template v-if="isHome">
      ...
      <section class="board-section" aria-labelledby="board-title">
        ...
        <div class="tier-board"><section v-for="(tier, index) in tiers" :key="tier.id" class="tier-row" :style="{ '--tier-color': tier.color, '--tier-pale': tier.pale }" :aria-labelledby="`tier-${tier.id}`"><div class="tier-label"><span class="tier-index">0{{ index + 1 }}</span><h3 :id="`tier-${tier.id}`">{{ tier.label }}</h3><span class="tier-count">{{ filtered.filter(p => p.tier === tier.id).length }} 个条目</span></div><div class="tier-content"><div v-if="!filtered.some(p => p.tier === tier.id)" class="tier-empty"><span class="empty-dash" aria-hidden="true"></span><span>暂无条目</span></div><div v-else class="card-grid"><PlaceCard v-for="p in filtered.filter(p => p.tier === tier.id)" :key="p.id" :place="p" :rank="site.rankWithinTier ? rankOf(p) : undefined" :badge="recentBadge(p.id, updates, site.recentDays)" /></div></div></section></div>
        <p class="board-caption"><span>推荐度从上到下递减</span><span>{{ site.rankWithinTier ? '同档分先后 · 从左到右，从上到下' : '同档不分先后' }}</span></p>
      </section>

      <div class="bottom-grid"><section class="updates-section" aria-labelledby="updates-title"><div class="section-heading"><div><span class="section-number">02</span><h2 id="updates-title">最近更新</h2></div><span class="muted tiny">持续记录中</span></div><div v-if="!updates.length" class="quiet-empty"><span class="timeline-dot"></span><div><h3>还没有更新记录</h3><p>新增、改档与评价修改，都会在这里留下足迹。</p></div></div><ol v-else class="update-list"><li v-for="u in updates.slice(0, 10)" :key="u.id"><time>{{ u.date }}</time><div><span class="update-kind">{{ updateLabel(u) }}</span><a :href="`#/place/${u.placeId}`">{{ all.find(p => p.id === u.placeId)?.name }}</a><span v-if="all.find(p => p.id === u.placeId)?.isDemo" class="demo-badge">演示</span><p v-if="u.type === 'tier-change'">{{ tierLabel(u.fromTier!) }} → {{ tierLabel(u.toTier!) }}</p><p>{{ u.note }}</p></div></li></ol></section>
    ...
    <article v-else-if="currentPlace" class="detail-page"><a href="#/" class="back-link">← 返回榜单</a><div class="detail-heading"><p class="eyebrow">{{ currentPlace.city }} · {{ tierLabel(currentPlace.tier) }}<span v-if="site.rankWithinTier"> · 档内第 {{ rankOf(currentPlace) }} 名</span><span v-if="currentPlace.isDemo" class="demo-badge">演示</span></p><h1>{{ currentPlace.name }}</h1><p class="detail-summary">{{ currentPlace.summary }}</p><div class="muted">到访 · {{ currentPlace.visitedAt || '未记录' }}<br />内容更新 · {{ currentPlace.updatedAt }}</div></div><div class="detail-photo"><PlaceImage :src="currentPlace.cover" :alt="currentPlace.coverAlt" :position="currentPlace.coverPosition" /></div><div class="detail-body"><div class="tags"><span v-for="tag in currentPlace.tags" :key="tag">{{ tag }}</span></div><div v-if="currentPlace.pros || currentPlace.cons" class="pros-cons"><section v-if="currentPlace.pros"><h2>值得一提</h2><p>{{ currentPlace.pros }}</p></section><section v-if="currentPlace.cons"><h2>有点遗憾</h2><p>{{ currentPlace.cons }}</p></section></div><h2>详细体验</h2><p class="detail-text">{{ currentPlace.details || '详细体验待补充。' }}</p><figure v-for="image in currentPlace.gallery" :key="image.src"><PlaceImage :src="image.src" :alt="image.alt" /><figcaption>{{ image.alt }}</figcaption></figure></div><a href="#/" class="back-link">← 返回榜单，继续看看</a></article>
